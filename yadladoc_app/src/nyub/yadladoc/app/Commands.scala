...
object YdocMain extends Command:
    override def parse(args: Seq[String]): Command = args match
        case "check" :: rest => Check(Seq.empty).parse(rest)
        case "run" :: rest   => Run(Seq.empty).parse(rest)
...
case class Check(files: Seq[Path]) extends Command:
    override def parse(args: Seq[String]): Command =
    ...
    override def run(): ExitCode =
        ...
        withYadladoc: yadladoc =>
            ...
            printErrorsThenExitCode(errors, AnsiPrinter.NO_COLOR)

case class Run(files: Seq[Path]) extends Command:
    override def parse(args: Seq[String]): Command =
        if files.isEmpty && args.isEmpty then Help(true)
        else this.copy(files = files ++ args.map(Paths.get(_)))
