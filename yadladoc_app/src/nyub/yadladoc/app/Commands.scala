...
case class Help(
    ...
    override def parse(args: Seq[String]): Command = args match
        case Nil                  => this
        case "properties" :: rest => PrintProperties(this.options).parse(rest)
        case _                    => this.copy(failureMode = true)

    override def run(): ExitCode =
        options.printer.println(help)
        if failureMode then INVALID_ARGUMENT_RETURN_CODE else OK_RETURN_CODE

    private val help = """Usage: yadladoc {run|check|help} [markdown_files, ...]
    ...
    help : print this message
        help properties: print yadladoc properties gathererd from env and command line
Common options:
...
case class PrintProperties(options: CommonOptions = CommonOptions.DEFAULTS)
    extends Command:
    override def parse(args: Seq[String]): Command =
    ...
    override def run(): ExitCode =
        ...
        if !configDir.toFile().isDirectory() then
            ...
            val config = ConfigurationFromFile(
              configDir,
              ConfigurationConstants.DEFAULTS,
              overrideProperties = options.overrideProperties
            )
            config.properties.toMap.toList.sorted.foreach: (key, value) =>
                options.printer.println(s"'$key' -> '$value'")
            OK_RETURN_CODE
